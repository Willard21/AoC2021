...
for (let i = 1; i < 5; i++) input.forEach(row => {
	let copy = row.slice(0, width)
	row.push(...copy.map(n => n + i > 9 ? n + i - 9 : n + i))
})
for (let i = 1; i < 5; i++) {
	let copy = input.slice(0, width)
	input.push(...copy.map(row => row.map(n => n + i > 9 ? n + i - 9 : n + i)))
}
